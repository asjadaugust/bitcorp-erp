...
UI Generation & Consistency RulesWhenever you are asked to generate, modify, or review Frontend UI code (Angular), you MUST strictly adhere to the following rules. Failure to do so will break the application's design system.1. Context Gathering (MANDATORY FIRST STEP)Before writing any HTML or SCSS for a new module or component, you must first read and analyze a "Gold Standard" reference module to ensure your layout matches.If building a List/Data page: Read frontend/src/app/features/equipment/equipment-list.component.ts (and its HTML).If building a Detail page: Read frontend/src/app/features/equipment/equipment-detail/equipment-detail.component.ts.If building a Form: Read frontend/src/app/features/equipment/equipment-form.component.ts.2. Layout WrappersNEVER build a raw <div> layout for a page. You must use the established layout shells:Standard Pages: Wrap the entire page in <app-page-layout>. Use <app-page-card> for the main content area.Detail/Entity Pages: Wrap the page in <app-entity-detail-shell>.3. Design System Components (Aero)Do NOT use raw HTML elements for interactive UI. Map standard HTML to our custom Aero design system:Buttons: Use <aero-button> instead of <button>. (Check frontend/src/app/core/design-system/button/aero-button.component.ts for inputs).Cards: Use <aero-card> or <app-page-card> instead of div.bg-white.shadow-md.Inputs/Forms: Use <aero-input> instead of raw <input class="...">.Select/Dropdowns: Do NOT use raw <select>. Use <app-dropdown> (found in frontend/src/app/shared/components/dropdown/dropdown.component.ts).Badges/Tags: Use <aero-badge>.Tabs: Use <aero-tabs>.4. Navigation & HeadersNo local nav bars: Individual modules should NEVER implement their own top navigation bars.All top-level navigation is handled by <app-main-nav> (frontend/src/app/shared/components/main-nav.component.ts).If a module requires secondary navigation, use <app-module-nav> or <aero-tabs>. Do not invent a new navbar.5. Styling & ColorsNo hardcoded colors: NEVER use hardcoded hex codes (e.g., #3b82f6) or arbitrary Tailwind colors (e.g., bg-blue-500) unless explicitly instructed.Use Tokens: You must rely on the CSS variables defined in frontend/src/styles/klm-tokens.css (e.g., var(--klm-primary-blue)).Spacing: Stick to Tailwind's standard spacing scale (p-4, m-6, gap-4).6. Dialogs & ModalsDo not use window.confirm or alert.Use <app-confirm-dialog> or <aero-dialog> triggered via the Angular CDK Overlay/Dialog service.Verification StepBefore finalizing your code, ask yourself:Did I use aero- components?Is this page wrapped in <app-page-layout> or <app-entity-detail-shell>?Did I accidentally create a new navigation bar?
...
- **Inputs**: `<aero-input>` for text/number/date, `<app-dropdown>` for selects, raw `<textarea class="form-control">` for multi-line text

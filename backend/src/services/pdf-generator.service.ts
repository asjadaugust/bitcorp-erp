...
export class PDFGeneratorService {
  async generateDailyReportPDF(data: DailyReportPDFData): Promise<Buffer> {
    return new Promise((resolve, reject) => {
      ...
      this.addRow(doc, 'Diferencia Horómetro:', 
      ...
      if (data.odometer_start !== undefined && data.odometer_end !== undefined) {
        ...
        this.addRow(doc, 'Distancia Recorrida:', 
      ...
      if (data.fuel_start !== undefined && data.fuel_end !== undefined) {
        ...
        if (data.fuel_consumed !== undefined) {
          this.addRow(doc, 'Combustible Consumido:', 
            `${data.fuel_consumed.toFixed(1)}%`);
        }
        doc.moveDown(1);
      }
      ...
      doc.moveTo(70, signatureY)
         ...
         .stroke();
      doc.fontSize(9)
         .text('Firma del Operador', 70, signatureY + 5);
      ...
      doc.moveTo(350, signatureY)
         ...
         .stroke();
      doc.fontSize(9)
         .text('Firma del Supervisor', 350, signatureY + 5);
  ...
  async generateBatchReportsPDF(reports: DailyReportPDFData[]): Promise<Buffer> {
    return new Promise((resolve, reject) => {
      ...
      if (reports.length > 0) {
        ...
        doc.text(`Período: ${this.formatDate(startDate)} - ${this.formatDate(endDate)}`, 
          { align: 'center' })
           .moveDown(2);
      }
      ...
      const totalHourmeter = reports.reduce((sum, r) => 
        sum + (r.hourmeter_end - r.hourmeter_start), 0);
  ...
  private generateSingleReportPage(doc: PDFKit.PDFDocument, data: DailyReportPDFData): void {
    ...
    this.addRow(doc, 'Horómetro:', 
      `${data.hourmeter_start.toFixed(1)} → ${data.hourmeter_end.toFixed(1)} hrs`, 9);
    if (data.fuel_consumed !== undefined) {
    ...
    const workText = data.work_description.length > 200 
  ...
  private addRow(doc: PDFKit.PDFDocument, label: string, value: string, fontSize: number = 10): void {
    const x = doc.x;
    const y = doc.y;

...
export class PdfService {
  generateValuationPdf(valuation: any, res: Response) {
    ...
    doc
      .fontSize(12)
      .text(`Mes: ${valuation.period_month} / ${valuation.period_year}`, { align: 'center' });
    ...
    doc
      .fillColor('black')
      .font('Helvetica-Bold')
      .text('INFORMACIÓN DEL PROVEEDOR', 60, doc.y + 5);
    doc.moveDown(2);
    ...
    doc
      .fillColor('black')
      .font('Helvetica-Bold')
      .text('PERIODO DE VALORIZACIÓN', 60, doc.y + 5);
    doc.moveDown(2);
    ...
    doc.fontSize(10);
    doc.text(
      `Costo Base: ${valuation.moneda || 'PEN'} ${Number(valuation.costo_base || 0).toFixed(2)}`
    );
    doc.text(
      `Costo Exceso: ${valuation.moneda || 'PEN'} ${Number(valuation.costo_horas_exceso || 0).toFixed(2)}`
    );
    doc.text(
      `Costo Combustible: ${valuation.moneda || 'PEN'} ${Number(valuation.costo_combustible || 0).toFixed(2)}`
    );
    doc.text(
      `Cargos Adicionales: ${valuation.moneda || 'PEN'} ${Number(valuation.cargos_adicionales || 0).toFixed(2)}`
    );
    doc.moveDown();
    doc
      .font('Helvetica-Bold')
      .fontSize(14)
      .text(
        `TOTAL: ${valuation.moneda || 'PEN'} ${Number(valuation.total_valorizado || valuation.amount || 0).toFixed(2)}`,
        { align: 'right' }
      );

...
export class AdministrationService {
  ...
  async createCostCenter(data: Partial<CostCenter>): Promise<CostCenter> {
    const query = `
      INSERT INTO administracion.centro_costo (
        codigo, nombre, descripcion, proyecto_id, presupuesto, is_active
  ...
  async updateCostCenter(id: string, data: Partial<CostCenter>): Promise<CostCenter | null> {
    ...
    if (data.codigo) {
      clauses.push(`codigo = $${idx++}`);
      values.push(data.codigo);
    }
    if (data.nombre) {
      clauses.push(`nombre = $${idx++}`);
      values.push(data.nombre);
    }
    if (data.descripcion) {
      clauses.push(`descripcion = $${idx++}`);
      values.push(data.descripcion);
    }
    if (data.projectId !== undefined) {
      clauses.push(`proyecto_id = $${idx++}`);
      values.push(data.projectId);
    }
    if (data.presupuesto !== undefined) {
      clauses.push(`presupuesto = $${idx++}`);
      values.push(data.presupuesto);
    }

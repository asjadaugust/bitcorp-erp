...
test.describe('Daily Reports - Complete Feature Tests', () => {
  ...
  test('BITCORP-002: Create daily report with all fields', async ({ page }) => {
    ...
    await page.fill(
      ...
      'Equipment performed well. Minor vibration noted.'
    );
  ...
  test('BITCORP-002: Photo upload functionality', async ({ page }) => {
    ...
    await fileInput.setInputFiles(
      '/Users/klm95441/Library/CloudStorage/SynologyDrive-projects/bitcorp-erp/tests/test-data/test-image.jpg'
    );
  ...
  test('BITCORP-002: GPS location capture', async ({ page, context }) => {
    ...
    await expect(page.locator('input[formControlName="gps_latitude"]')).toHaveValue(/18\.46/, {
      timeout: 5000,
    });
  ...
  test('BITCORP-003: Equipment checklist integration', async ({ page }) => {
    ...
    const checkedCount = await page
      .locator('.checklist-item input[type="checkbox"]:checked')
      .count();
  ...
  test('BITCORP-003: Form validation', async ({ page }) => {
    ...
    await expect(
      page.locator('.error-message:has-text("End hourmeter must be greater than start")')
    ).toBeVisible();
  });
  ...
  test('BITCORP-003: View and edit existing report', async ({ page }) => {
    ...
    await page.fill(
      'textarea[formControlName="notes"]',
      'Updated notes - Equipment maintenance scheduled'
    );
  ...
  test('BITCORP-003: Calculate working hours automatically', async ({ page }) => {
    ...
    await expect(page.locator('input[formControlName="hours_worked"]')).toHaveValue('9.5', {
      timeout: 2000,
    });
  });
...
test.describe('Daily Reports - Error Handling', () => {
  ...
  test('Handle network error gracefully', async ({ page, context }) => {
    ...
    await expect(page.locator('.error-message:has-text("Network error")')).toBeVisible({
      timeout: 5000,
    });
  ...
  test('Handle duplicate report submission', async ({ page }) => {
    ...
    await expect(page.locator('.warning-message:has-text("duplicate")')).toBeVisible({
      timeout: 5000,
    });
  });
});

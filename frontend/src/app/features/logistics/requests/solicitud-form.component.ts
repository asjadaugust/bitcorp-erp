...
import { SolicitudMaterialService, SolicitudMaterialDetalle } from './solicitud-material.service';
...
@Component({
  ...
  template: `
    <app-form-container
      [title]="isEditMode ? 'Editar Solicitud de Material' : 'Nueva Solicitud de Material'"
      [subtitle]="
        isEditMode
          ? 'Actualizar la solicitud de material'
          : 'Registrar una nueva solicitud de material'
      "
      [icon]="isEditMode ? 'fa-pen' : 'fa-file-lines'"
      [loading]="loading"
      [disableSubmit]="form.invalid || loading"
      [submitLabel]="isEditMode ? 'Guardar Cambios' : 'Crear Solicitud'"
      (submitted)="onSubmit()"
      (cancelled)="cancel()"
    >
      <form [formGroup]="form" class="form-grid">
        <!-- Datos de la Solicitud -->
        <app-form-section title="Datos de la Solicitud" icon="fa-file-lines">
        ...
        <app-form-section title="Detalle de la Solicitud" icon="fa-list-ol" [columns]="1">
          <div class="detalles-header">
            <p class="detalles-info">Agregue las lineas de detalle de la solicitud.</p>
            <aero-button
              variant="secondary"
              size="small"
              iconLeft="fa-plus"
...
export class SolicitudFormComponent implements OnInit {
  ...
  loadSolicitud(id: number): void {
    ...
    this.solicitudService.getSolicitud(id).subscribe({
      next: (solicitud: SolicitudMaterialDetalle) => {
      ...
      error: () => {
        this.loading = false;
        this.router.navigate(['/logistics/material-requests']);
      },
    });
  }
  ...
  onSubmit(): void {
    ...
    if (this.isEditMode && this.solicitudId) {
      ...
      this.solicitudService.updateSolicitud(this.solicitudId, headerData).subscribe({
        next: () => {
          this.router.navigate(['/logistics/material-requests']);
        },
        error: () => {

...
@Component({
  ...
  template: `
    <div class="reports-container">
      ...
      <div class="card filter-card">
        <div class="filter-grid">
          ...
          <i class="fa-solid fa-table"></i> Ver Datos
            ...
            <button type="button" class="btn btn-success" (click)="exportExcel()" [disabled]="loading">
      ...
      <div *ngIf="!loading && reportData && selectedReport === 'utilization'" class="report-result card">
        <h3>Reporte de Utilización</h3>
        <aero-table [columns]="columnsUtilization" [data]="reportData"></aero-table>
      </div>

      <!-- Maintenance Report Table -->
      <div *ngIf="!loading && reportData && selectedReport === 'maintenance'" class="report-result card">
      ...
      <div *ngIf="!loading && reportData && selectedReport === 'inventory'" class="report-result card">
        ...
        <ng-template #movementTypeTemplate let-row>
          <span [class]="'badge ' + (row.tipo_movimiento === 'IN' ? 'badge-success' : 'badge-warning')">
            {{ row.tipo_movimiento }}
          </span>
        </ng-template>
      </div>
      ...
      <div *ngIf="!loading && reportData && selectedReport === 'timesheet'" class="report-result card">
  ...
  styles: [`
    ...
    .header h1 { font-size: 24px; color: var(--primary-900); margin: 0; }
    .header p { color: var(--grey-500); margin: 4px 0 0; }
    ...
    label { font-size: 14px; font-weight: 500; color: var(--grey-700); }
    ...
    .btn-success { background: var(--semantic-green-600); }
...
export class ReportsComponent {
  ...
  columnsUtilization: TableColumn[] = [
    { key: 'equipment_code', label: 'Equipo', type: 'text' },
    { key: 'date', label: 'Fecha', type: 'date', format: 'dd/MM/yyyy' },
    { key: 'operator_name', label: 'Operador', type: 'text' },
    { key: 'project_name', label: 'Proyecto', type: 'text' },
  ...
  columnsMaintenance: TableColumn[] = [
    ...
    { key: 'equipment_name', label: 'Equipo', type: 'text' },
  ...
  columnsTimesheet: TableColumn[] = [
    ...
    { key: 'project_name', label: 'Proyecto Principal', type: 'text' },
    { key: 'total_hours', label: 'Horas Totales', type: 'text' },
  ...
  exportExcel() {
    ...
    switch (this.selectedReport) {
      case 'utilization':
        request = this.reportingService.getEquipmentUtilization(this.startDate, this.endDate, 'excel');
        break;
      case 'maintenance':
        request = this.reportingService.getMaintenanceHistory(this.startDate, this.endDate, 'excel');
        break;
      case 'inventory':
        request = this.reportingService.getInventoryMovements(this.startDate, this.endDate, 'excel');
        break;
      case 'timesheet':

...
export class DailyReportFormPWAComponent implements OnInit {
  ...
  async onPhotoSelected(event: Event): Promise<void> {
    ...
    try {
      ...
      if (response.success && response.data) {
        ...
        this.report.photos = this.photos().map((p: { url: string; filename: string; size: number }) => p.url);
  ...
  async removePhoto(index: number): Promise<void> {
    ...
    try {
      ...
      const updatedPhotos = this.photos().filter((_: { url: string; filename: string; size: number }, i: number) => i !== index);
      this.photos.set(updatedPhotos);
      this.report.photos = updatedPhotos.map((p: { url: string; filename: string; size: number }) => p.url);
    } catch (error) {

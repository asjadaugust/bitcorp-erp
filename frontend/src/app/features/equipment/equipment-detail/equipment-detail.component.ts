...
@Component({
                  ...
                  <table class="aero-table">
                    ...
                    <tbody>
                      <tr *ngFor="let contract of contracts">
                        ...
                        <td>{{ contract.start_date | date:'shortDate' }} - {{ contract.end_date | date:'shortDate' }}</td>
                        <td>
                  ...
                  <table class="aero-table">
                    ...
                    <tbody>
                      <tr *ngFor="let report of dailyReports">
                        ...
                        <td>{{ (report.hourmeter_end - report.hourmeter_start) | number:'1.1-1' }}</td>
                        <td>
  ...
  styles: [`
    ...
    .status-icon { background: #e6fffa; color: #38b2ac; }
    .hour-icon { background: #ebf8ff; color: #4299e1; }
    .project-icon { background: #faf5ff; color: #9f7aea; }
    .provider-icon { background: #fffaf0; color: #ed8936; }
    ...
    .status-available, .status-active { background: #c6f6d5; color: #22543d; }
    .status-maintenance { background: #feeebc; color: #744210; }
    .status-assigned { background: #bee3f8; color: #2a4365; }
    .status-inactive { background: #edf2f7; color: #4a5568; }
...
export class EquipmentDetailComponent implements OnInit {
  ...
  loadRelatedData(id: number) {
    ...
    this.dailyReportService.getAll({ equipment_id: id }).subscribe({
      next: (reports) => {
    ...
    this.contractService.getAll({ equipmentId: id }).subscribe({
      next: (contracts) => {
    ...
    this.maintenanceService.getAll({ equipment_id: id }).subscribe({
      next: (res: any) => {
        this.maintenanceSchedules = res.data || [];
        this.checkLoading();
      },
      error: () => this.checkLoading()
    });
  ...
  createContract() {
    this.router.navigate(['/equipment/contracts/new'], { queryParams: { equipmentId: this.equipment?.id } });
  }

  goToMaintenance() {
    this.router.navigate(['/equipment/maintenance'], { queryParams: { equipment_id: this.equipment?.id } });
  }

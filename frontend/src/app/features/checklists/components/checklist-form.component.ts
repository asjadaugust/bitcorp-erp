...
export class ChecklistFormComponent implements OnInit {
  ...
  items: Array<ChecklistItem & { is_required: boolean; expected_value?: string; allow_photos: boolean; allow_comments: boolean }> = [];
  observations = '';
  ...
  updateOverallStatus() {
    ...
    } else if (statuses.every((s) => s === ChecklistStatus.PASS || s === ChecklistStatus.NOT_APPLICABLE)) {
  ...
  onSave() {
    ...
    const dto: CreateChecklistDto = {
      ...
      items: this.items.map(({ id, is_required, expected_value, allow_photos, allow_comments, ...item }) => item),

...
import { Observable, BehaviorSubject, tap, switchMap, of, shareReplay, map, catchError } from 'rxjs';
...
export class AuthService {
  ...
  ensureUserLoaded(): Observable<boolean> {
    ...
    this.userLoadObservable = this.http.get<{ user: User }>(`${this.apiUrl}/me`).pipe(
      switchMap(response => {
  ...
  login(credentials: LoginRequest): Observable<AuthResponse> {
    ...
    return this.http.post<AuthResponse>(`${this.apiUrl}/login`, credentials).pipe(
      tap((response) => {
        ...
        try {
          console.log('User roles type:', typeof response.user.roles, Array.isArray(response.user.roles));

  ...
  animations: [
    trigger('tabItemAnim', [
      transition(':enter', [
        ...
        animate('300ms ease-out', style({ width: '*', opacity: 1, paddingLeft: '*', paddingRight: '*', overflow: 'hidden' }))
      ]),
      transition(':leave', [
        animate('250ms ease-in', style({ width: 0, opacity: 0, paddingLeft: 0, paddingRight: 0, overflow: 'hidden' }))
      ])
    ])
  ],
  template: `

...
test.describe('Daily Report Photo Upload', () => {
  ...
  test('should show photo upload section', async ({ page }) => {
    ...
    await expect(
      page.locator('button:has-text("Capturar Foto"), label:has-text("Agregar Foto")')
    ).toBeVisible();
  });
  ...
  test('should show progress during upload', async ({ page }) => {
    ...
    await expect(page.locator('.upload-progress, .progress-bar, text=Cargando')).toBeVisible({
      timeout: 3000,
    });
  });
  ...
  test('should limit to 5 photos maximum', async ({ page }) => {
    ...
    const addButton = page.locator(
      'button:has-text("Capturar Foto"), button:has-text("Agregar Foto"), label:has-text("Agregar Foto")'
    );
  ...
  test('should allow removing uploaded photos', async ({ page }) => {
    ...
    const removeButton = page
      .locator('.remove-photo, button[title="Eliminar"], button:has-text("×")')
      .first();
    await removeButton.click();

    // Photo should be removed
    await expect(page.locator('.photo-item img, .photo-grid img').first()).not.toBeVisible({
      timeout: 2000,
    });
  });
  ...
  test('should show status indicators', async ({ page }) => {
    ...
    const statusElements = page.locator(
      '.photo-status, .status-badge, text=/Local|Guardado|Cargando/'
    );

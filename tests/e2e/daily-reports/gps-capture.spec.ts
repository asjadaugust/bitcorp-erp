...
test.describe('GPS Geolocation Capture', () => {
  ...
  test('should capture GPS coordinates on button click', async ({ page }) => {
    ...
    const gpsInput = page.locator(
      '[formControlName="gpsLocation"], input[placeholder*="GPS"], input[placeholder*="ubicación"]'
    );
  ...
  test('should handle GPS permission denied', async ({ page, context }) => {
    ...
    await expect(
      page.locator('text=/Permiso.*denegado|GPS.*error|ubicación.*denegado/i')
  ...
  test('should allow manual location entry as fallback', async ({ page }) => {
    ...
    const manualInput = page.locator(
      '[formControlName="manualLocation"], input[placeholder*="manual"], input[placeholder*="Descripción"]'
    );
  ...
  test('should show Google Maps link in view mode', async ({ page }) => {
    ...
    const reportLinks = page.locator(
      'a[href*="/operator/history/"], button:has-text("Ver"), .report-item'
    );
    ...
    if (count > 0) {
      ...
      const mapsLink = page.locator(
        'a[href*="google.com/maps"], button:has-text("Ver en Mapa"), button:has-text("🗺️")'
      );

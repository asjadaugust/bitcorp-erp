...
test.describe('Operator Login Verification', () => {
  ...
  test('should login successfully with operator1 credentials and access operator dashboard', async ({ page }) => {
    // Fill in login form
    ...
    await expect(page.locator('.operator-layout, .operator-dashboard')).toBeVisible({ timeout: 10000 });
    ...
    const hasOperadorText = await page.locator('text=/Panel de Operador|Operador|Dashboard/i').isVisible();
    expect(hasOperadorText).toBe(true);
    
    // Verify navigation menu items are present
    const hasPanelLink = await page.locator('a[href*="operator/dashboard"], a:has-text("Panel")').count();
  ...
  test('should navigate to daily report form from operator dashboard', async ({ page }) => {
    ...
    const dailyReportLink = page.locator(
      'a[href*="daily-report"], a:has-text("Parte Diario"), a:has-text("Nuevo Parte")'
    ).first();
  ...
  test('should logout successfully from operator dashboard', async ({ page }) => {
    ...
    const logoutButton = page.locator(
      'button:has-text("Cerrar Sesión"), button:has-text("Logout"), .logout-btn'
    ).first();
  ...
  test('should verify operator role is correctly set', async ({ page, context }) => {
    ...
    if (userDataString) {
      ...
      const hasOperatorRole = userData.roles && 

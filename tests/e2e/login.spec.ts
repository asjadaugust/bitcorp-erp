...
test.describe('Login Tests', () => {
  ...
  test('should login successfully with admin credentials', async ({ page }) => {
    ...
    await expect(page.locator('.control-panel, .dashboard-container, h1').first()).toBeVisible({ timeout: 10000 });
  ...
  test('should show error with invalid credentials', async ({ page }) => {
    ...
    const hasErrorClass = await page.locator('.alert-error').isVisible().catch(() => false);
    const hasErrorMessage = await page.locator('text=/invalid|wrong|incorrect|failed/i').isVisible().catch(() => false);
